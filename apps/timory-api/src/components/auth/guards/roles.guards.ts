...
import { Message } from 'apps/timory-api/src/libs/enums/common.enum';
...
export class RolesGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext | any): Promise<boolean> {
		const roles = this.reflector.get<string[]>('roles', context.getHandler()); // bu yerda ADMIN, AGENT yoki MEMBER ekanligi tekshiriladi
		...
		if (context.contextType === 'graphql') {
			...
			console.log('memberName[roles] =>', authMember.memberName);
